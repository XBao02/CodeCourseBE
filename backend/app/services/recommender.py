...
def recommend_courses(level=None, major=None, topic=None, limit=6):
    """
    ...
    if len(scored) < 3:
        # Relax level but keep major/topic
        q3 = Course.query
        if major:
    ...
    if len(result) < 3:
        rest = Course.query.order_by(Course.created_at.desc()).limit(10).all()
