...
from ..models.model import db, Course, Instructor, CourseSection, Lesson, Test, Question, Choice
import re
...
def get_test(test_id):
    ...
    res = {
        'id': test.id,
        'title': test.title,
        'timeLimitMinutes': test.time_limit_minutes if hasattr(test, 'time_limit_minutes') else getattr(test, 'timeLimitMinutes', None),
        'attemptsAllowed': test.attempts_allowed if hasattr(test, 'attempts_allowed') else getattr(test, 'attemptsAllowed', 1),
        'isPlacement': bool(getattr(test, 'is_placement', getattr(test, 'isPlacement', False)))
    }
    ...
    for q in Question.query.filter_by(test_id=test.id).all():
        ...
        for c in Choice.query.filter_by(question_id=q.id).all():
            q_json['choices'].append({ 'id': c.id, 'text': c.text, 'isCorrect': bool(getattr(c, 'is_correct', getattr(c, 'isCorrect', False))) })
